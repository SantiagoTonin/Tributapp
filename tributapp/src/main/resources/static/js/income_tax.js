...
$(document).ready(function () {
  ...
  $("#select_year").on("change", function () {
    ...
    if (is_patagonia === TRUE) {
      gmni = CONSTANTS1[year_val].GMNI_PATAGONIA;
      special_deduction = CONSTANTS1[year_val].DEDUCCION_ESPECIAL_EMPLEADO_PATAGONIA;
      div_gmni.textContent = "$".concat(" ", gmni, " al año");
    } else if (is_patagonia === FALSE) {
      gmni = CONSTANTS1[year_val].GMNI;
      special_deduction = CONSTANTS1[year_val].DEDUCCION_ESPECIAL_EMPLEADO;
      div_gmni.textContent = "$".concat(" ", gmni, " al año");
    }
  });
  ...
  $("#button_calculate").on("click", function () {
    ...
    if (can_deduct_spouse === TRUE) {
      if (is_patagonia === TRUE) {
        spouse_deduction = CONSTANTS1[year_val].DEDUCCION_CONYUGE_PATAGONIA;
      } else if (is_patagonia === FALSE) {
        spouse_deduction = CONSTANTS1[year_val].DEDUCCION_CONYUGE;
      }
    ...
    if (is_patagonia === TRUE) {
      child_deduction = child_deduction_factor * CONSTANTS1[year_val].DEDUCCION_POR_HIJO_PATAGONIA;
    } else if (is_patagonia === FALSE) {
      child_deduction = child_deduction_factor * CONSTANTS1[year_val].DEDUCCION_POR_HIJO;
    }
    ...
    if (life_insurance_deduction >= CONSTANTS1[year_val].TOPE_SEGUROS) {
      life_insurance_deduction = CONSTANTS1[year_val].TOPE_SEGUROS;
    }
    var retirement_insurance_deduction = $("#seguro_retiro").val() * MONTHS;
    if (retirement_insurance_deduction >= CONSTANTS1[year_val].TOPE_SEGUROS) {
      retirement_insurance_deduction = CONSTANTS1[year_val].TOPE_SEGUROS;
    }

    //This one also has his own max deduction
    var mortgage_credit_interest_deduction = $("#intereses_creditos").val() * MONTHS;
    if (mortgage_credit_interest_deduction >= CONSTANTS1[year_val].TOPE_INTERESES) {
      mortgage_credit_interest_deduction = CONSTANTS1[year_val].TOPE_INTERESES;
    }
    ...
    } else if (tax_base < CONSTANTS1[year_val].TABLA_ALICUOTA_TOPES[SIZE]) {
      while (yearly_payement === ZERO) {
        if (
          tax_base >= CONSTANTS1[year_val].TABLA_ALICUOTA_TOPES[i] &&
          tax_base < CONSTANTS1[year_val].TABLA_ALICUOTA_TOPES[i + 1]
        ) {
          yearly_payement =
            CONSTANTS1[year_val].TABLA_ALICUOTA_BASES[i] +
            (tax_base - CONSTANTS1[year_val].TABLA_ALICUOTA_TOPES[i]) *
              CONSTANTS1[year_val].TABLA_ALICUOTA_PORCENTAJES[i];
        }
        i++;
      }
    } else if (tax_base >= CONSTANTS1[year_val].TABLA_ALICUOTA_TOPES[SIZE]) {
      yearly_payement =
        CONSTANTS1[year_val].TABLA_ALICUOTA_BASES[SIZE] +
        (tax_base - CONSTANTS1[year_val].TABLA_ALICUOTA_TOPES[SIZE]) *
          CONSTANTS1[year_val].TABLA_ALICUOTA_PORCENTAJES[SIZE];
    }
