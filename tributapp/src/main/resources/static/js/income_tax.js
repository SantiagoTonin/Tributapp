...
$(document).ready(function() {
    ...
    var gmni = Constantes[20211].GMNI;
    ...
    $("#select_year").on("change", function() {
        ...
        if (is_patagonia === TRUE) {
            gmni = Constantes[year_val].GMNI_PATAGONIA;
            special_deduction = Constantes[year_val].DEDUCCION_ESPECIAL_EMPLEADO_PATAGONIA;
            div_gmni.textContent = "$".concat(" ", gmni, " al año");
        } else if (is_patagonia === FALSE) {
            gmni = Constantes[year_val].GMNI;
            special_deduction = Constantes[year_val].DEDUCCION_ESPECIAL_EMPLEADO;
            div_gmni.textContent = "$".concat(" ", gmni, " al año");
        }
        monthly_salary = parseInt(document.getElementById("sueldo_bruto").value);
        if (monthly_salary === ZERO) {
            $("#no_tax_message").hide();
        } else if (monthly_salary <= Constantes[year_val].SUELO_IMPUESTO) {
            $("#no_tax_message").show();
        } else {
            $("#no_tax_message").hide();
        }
    ...
    $("#button_calculate").on("click", function() {
        ...
        if (monthly_salary <= Constantes[year_val].SUELO_IMPUESTO) {
            special_deduction_table = NO_TAX;
        } else if (monthly_salary <= Constantes[year_val].SUELO_DEDUCCION_ESPECIAL) {
            for (let idx = 0; idx < Constantes[year_val].SUELDO_BRUTO_DEDUCCION_ESPECIAL.length; idx++) {
                if (Constantes[year_val].SUELDO_BRUTO_DEDUCCION_ESPECIAL[idx] >= monthly_salary) {
                    special_deduction_table = Constantes[year_val].DEDUCCION_ESPECIAL[idx];
                    break;
                }
            }
        }
        ...
        if (can_deduct_spouse === TRUE) {
            if (is_patagonia === TRUE) {
                spouse_deduction = Constantes[year_val].DEDUCCION_CONYUGE_PATAGONIA;
            } else if (is_patagonia === FALSE) {
                spouse_deduction = Constantes[year_val].DEDUCCION_CONYUGE;
            }
        ...
        if (is_patagonia === TRUE) {
            child_deduction = child_deduction_factor * Constantes[year_val].DEDUCCION_POR_HIJO_PATAGONIA;
        } else if (is_patagonia === FALSE) {
            child_deduction = child_deduction_factor * Constantes[year_val].DEDUCCION_POR_HIJO;
        }
        ...
        if (life_insurance_deduction >= Constantes[year_val].TOPE_SEGUROS) {
            life_insurance_deduction = Constantes[year_val].TOPE_SEGUROS;
        }
        var retirement_insurance_deduction = $("#seguro_retiro").val() * MONTHS;
        if (retirement_insurance_deduction >= Constantes[year_val].TOPE_SEGUROS) {
            retirement_insurance_deduction = Constantes[year_val].TOPE_SEGUROS;
        }

        //This one also has his own max deduction
        var mortgage_credit_interest_deduction = $("#intereses_creditos").val() * MONTHS;
        if (mortgage_credit_interest_deduction >= Constantes[year_val].TOPE_INTERESES) {
            mortgage_credit_interest_deduction = Constantes[year_val].TOPE_INTERESES;
        }
        ...
        } else if (tax_base < Constantes[year_val].TABLA_ALICUOTA_TOPES[SIZE]) {
            while (yearly_payement === ZERO) {
                if (
                    tax_base >= Constantes[year_val].TABLA_ALICUOTA_TOPES[i] &&
                    tax_base < Constantes[year_val].TABLA_ALICUOTA_TOPES[i + 1]
                ) {
                    yearly_payement =
                        Constantes[year_val].TABLA_ALICUOTA_BASES[i] +
                        (tax_base - Constantes[year_val].TABLA_ALICUOTA_TOPES[i]) *
                        Constantes[year_val].TABLA_ALICUOTA_PORCENTAJES[i];
                }
                i++;
            }
        } else if (tax_base >= Constantes[year_val].TABLA_ALICUOTA_TOPES[SIZE]) {
            yearly_payement =
                Constantes[year_val].TABLA_ALICUOTA_BASES[SIZE] +
                (tax_base - Constantes[year_val].TABLA_ALICUOTA_TOPES[SIZE]) *
                Constantes[year_val].TABLA_ALICUOTA_PORCENTAJES[SIZE];
        }
